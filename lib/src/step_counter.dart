// Step Counter with Kalman Filter Logic
import 'dart:async';
import 'dart:math';
...
class KalmanFilter {
  double _q = 0.0001; // process noise covariance
  double _r = 0.01;   // measurement noise covariance
  double _x = 0.0;    // value
...
class StepCounter {
  ...
  final Queue<double> _magnitudeBuffer = Queue();
  ...
  Future<void> _saveState() async {
    ...
    final timestampStrings = _todayStepTimestamps.map((dt) => dt.toIso8601String()).toList();
  ...
  Future<void> _loadState() async {
    ...
    _todayStepTimestamps.addAll(timestampStrings.map((s) => DateTime.tryParse(s)).whereType<DateTime>());
  ...
  void _cleanupOldTimestamps() {
    ...
    _todayStepTimestamps.removeWhere((ts) => now.difference(ts).inDays > 31);
  }

  double get caloriesBurned => _totalSteps * strideLengthMeters / 1000 * userWeightKg * 0.57;
  ...
  double get cadence {
    ...
    final duration = _todayStepTimestamps.last.difference(_todayStepTimestamps.first).inMinutes;
  ...
  void _startAccelerometerFallback() {
    accelerometerEventStream().listen((event) {
      final rawMagnitude = sqrt(event.x * event.x + event.y * event.y + event.z * event.z);
      final filteredMagnitude = _kalman.filter(rawMagnitude);

      _magnitudeBuffer.add(filteredMagnitude);
      if (_magnitudeBuffer.length > _bufferSize) {
        _magnitudeBuffer.removeFirst();
      }

      final avgMagnitude = _magnitudeBuffer.reduce((a, b) => a + b) / _magnitudeBuffer.length;

      final now = DateTime.now();
      if (avgMagnitude > 12.0 && (_lastStepTime == null || now.difference(_lastStepTime!) > _debounceDuration)) {
        _lastStepTime = now;
        _totalSteps++;
        _todayStepTimestamps.add(now);
        _cleanupOldTimestamps();
        _handleStatus(1);
        _emitStepData(now);
        _saveState();
  ...
  int get todaySteps {
    final now = DateTime.now();
    return _todayStepTimestamps.where((ts) => ts.year == now.year && ts.month == now.month && ts.day == now.day).length;
  }

  int get weeklySteps {
    final now = DateTime.now();
    final oneWeekAgo = now.subtract(const Duration(days: 7));
    return _todayStepTimestamps.where((ts) => ts.isAfter(oneWeekAgo) && ts.isBefore(now)).length;
  }

  int get monthlySteps {
    final now = DateTime.now();
    return _todayStepTimestamps.where((ts) => ts.year == now.year && ts.month == now.month).length;
  }
}
